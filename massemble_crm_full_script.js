...
function sendToMassembleCRMByRow(rowNumber) {
  try {
    ...
    const crmPayload = {
      ...
      phone: phone,
      gender: mapGender(rowData[IDX.gender]),
      birthDate: formatBirthDate(rowData[IDX.birthDate]),
      consultType: '보험상담',
      consultPath: '보탐정설문',
...
function getStatistics() {
  try {
    ...
    // 마셈블 CRM 전송 통계
    const crmSuccessCount = surveys.filter(r => String(r[13] || '').toLowerCase() === 'success').length;
    const crmPendingCount = surveys.filter(r => {
      const status = String(r[13] || '').toLowerCase();
      return status === 'pending' || !status;
    }).length;
    const crmErrorCount = surveys.filter(r => {
      const status = String(r[13] || '').toLowerCase();
    ...
    return {
      ...
      data:{
        ...
        massembleCRM: {
          ...
          totalProcessed: crmSuccessCount + crmErrorCount,
          needsProcessing: crmPendingCount
        },
        ...
        recentSurveys: surveys.slice(-5).map(r => ({ 
          time:r[0], 
          name:r[8]||'익명', 
          region:r[4], 
          score:r[11], 
          crmStatus: r[13] || 'pending' 
        })).reverse()
      },
...
function getRecentData(limit = 20) {
  try {
    ...
    if (!sh) return { error:'설문 응답 시트를 찾을 수 없습니다.' };
...
/** 크론: 미전송/실패건 주기적 처리 (마셈블 CRM 버전) */
function cronSendPendingToMassembleCRM() {
  const lock = LockService.getScriptLock();
  if (!lock.tryLock(10000)) return; // 동시 실행 방지
  
  try {
    Logger.log('🔄 자동 마셈블 CRM 전송 크론 시작');
    ...
    const headers = data[0];
    const statusIdx = headers.indexOf('CRM전송상태');
    const sentTimeIdx = headers.indexOf('CRM전송시간');
    
    if (statusIdx === -1) { 
      Logger.log('CRM전송상태 컬럼 없음'); 
      return; 
    }

    const now = Date.now();
    const SENDING_TIMEOUT_MS = 5 * 60 * 1000; // 5분 타임아웃
    let processedCount = 0;

    for (let r = 1; r < data.length; r++) {
      const row = data[r];
      let status = String(row[statusIdx] || '').trim().toLowerCase();
      const surveyId = row[1] || null;

      // 오래된 'sending' 상태를 fail로 복구
      if (status === 'sending' && sentTimeIdx !== -1) {
        const sentAt = row[sentTimeIdx];
        const sentAtMs = sentAt ? new Date(sentAt).getTime() : 0;
        if (!sentAt || (now - sentAtMs) > SENDING_TIMEOUT_MS) {
          sheet.getRange(r + 1, statusIdx + 1).setValue('fail');
          status = 'fail';
          Logger.log(`Timeout 복구: Row ${r + 1} sending → fail`);
        } else {
          continue; // 아직 진행 중
        }
      }

      // 미전송 또는 실패 상태인 경우 전송 시도
      if (status === '' || status === 'pending' || status === 'fail' || status === 'error' || status === 'api_error' || status === 'http_error') {
        try {
          if (surveyId) {
            sendToMassembleCRM(surveyId);
          } else {
            sendToMassembleCRMByRow(r + 1);
          }
          processedCount++;
          
          // 크론에서는 너무 많이 처리하지 않도록 제한 (5건)
          if (processedCount >= 5) break;
          
          Utilities.sleep(1000); // API 호출 간격
        } catch (error) {
          Logger.log(`크론 전송 오류 (Row ${r + 1}): ${error}`);
        }
      }
    }
    
    Logger.log(`🔄 자동 마셈블 CRM 전송 크론 완료: ${processedCount}건 처리`);
    
  } catch (e) {
    Logger.log('cronSendPendingToMassembleCRM 오류: ' + e);
  } finally {
    lock.releaseLock();
  }
}

/** 크론 트리거 생성 (마셈블 CRM 버전) */
function createCronTrigger() {
  try {
    // 기존 트리거 삭제
    ScriptApp.getProjectTriggers()
      .filter(t => t.getHandlerFunction() === 'cronSendPendingToMassembleCRM')
      .forEach(t => ScriptApp.deleteTrigger(t));

    // 새 트리거 생성 (1분마다)
    ScriptApp.newTrigger('cronSendPendingToMassembleCRM')
      .timeBased()
      .everyMinutes(1)
      .create();

    Logger.log('✅ 마셈블 CRM 자동 전송 트리거 생성됨 (1분 간격)');
    return '마셈블 CRM 자동 전송 트리거 생성 완료';
  } catch (error) {
...
function backfillMissingSurveyIds() {
  try {
    ...
    for (let r = 1; r < data.length; r++) {
      if (!data[r][1]) { // ID 컬럼이 비어있으면
        sh.getRange(r + 1, 2).setValue(generateId()); // 2번째 컬럼(ID)
        filledCount++;
      }
    }
    
    Logger.log(`✅ 누락된 설문 ID ${filledCount}개 채움`);
    return `누락된 설문 ID ${filledCount}개 채움`;
  } catch (error) {
    Logger.log('❌ ID 채우기 오류: ' + error);
    throw error;
  }
}
...
function testConnection() {
  try {
    ...
    Logger.log('✅ 연결된 시트: ' + sheets.join(', '));
    return '연결 테스트 성공: ' + sheets.join(', ');
  } catch (e) { 
    return '연결 테스트 실패: ' + e.message; 
  }
}
...
function testMassembleCRM() {
  ...
  try {
    ...
    Logger.log('📊 마셈블 CRM 상태 확인: ' + JSON.stringify(statusResult, null, 2));
    
    // 2. 첫 번째 미전송 데이터 전송 테스트
    const sendResult = sendToMassembleCRM(null);
    Logger.log('📤 마셈블 CRM 전송 테스트: ' + JSON.stringify(sendResult, null, 2));
    
    return {
      success: true,
      status: statusResult,
      send: sendResult,
      message: '마셈블 CRM 연동 테스트 완료'
    };
  } catch (error) {
    Logger.log('❌ 마셈블 CRM 테스트 오류: ' + error);
    return {
      success: false,
      error: error.message,
      message: '마셈블 CRM 연동 테스트 실패'
    };
  }
}

// 호환성을 위한 별칭 함수들 (기존 함수명 지원)
function sendToCRM(surveyId) { return sendToMassembleCRM(surveyId); }
function sendAllToCRM() { return sendAllToMassembleCRM(); }
function checkCRMStatus() { return checkMassembleCRMStatus(); }
function cronSendPendingToCRM() { return cronSendPendingToMassembleCRM(); }